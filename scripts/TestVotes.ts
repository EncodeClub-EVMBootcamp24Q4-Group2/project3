...
async function main() {
    ...
    console.log(
        `Account ${
          acc1.account.address
        } has ${formatEther(votes1AfterTransfer)} of voting power after transferring\n`
    );
    ...
    const votes2AfterDelegate = await contract.read.getVotes([
        acc2.account.address,
    ]);
    ...
    console.log(
        `Account ${
          acc2.account.address
        } has ${formatEther(votes2AfterDelegate)}  voting power after transferring\n`
    );
    ...
    for (let index = lastBlockNumber - 1n; index > 0n; index--) {
      ...
      console.log(
        `Account ${
          acc1.account.address
        } had ${formatEther(pastVotes)} voting power at block ${index}\n`
      );
    }
}
