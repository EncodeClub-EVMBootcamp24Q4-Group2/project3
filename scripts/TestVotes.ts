...
async function main() {
    ...
    const balanceBN = await (await contract).read.balanceOf([account1.account.address])
    console.log(
        `Account ${
          account1.account.address
        } has ${formatEther(balanceBN)} MyToken\n`
      );

    const votes = await contract.read.getVotes([account1.account.address]);
    console.log(
      `Account ${
        account1.account.address
    ...
    const delegateTx = await contract.write.delegate([account1.account.address], {
      ...
      const votesAfter = await contract.read.getVotes([account1.account.address]);
      console.log(
        `Account ${
            account1.account.address
  ...
  const votesAfter2 = await contract.read.getVotes([account2.account.address]);
  console.log(
    `Account ${
        account2.account.address
